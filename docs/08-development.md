...
## 当前可运行的 GBA 图鉴（2026-09-20）

已新增真实 GBA 图鉴 ROM、中文界面、双槽 SRAM 与浏览器 mGBA 运行器。构建方式、依赖、验收和游戏接入契约见 [GBA 图鉴与培养方案](13-gba-dex-training.md)。本机已通过 Zig 编译和模拟器执行；完整冒险游戏与真机验证仍未完成。

以下保留项目初始化时的环境调查和上游引擎候选验证步骤，其“未执行游戏”等状态只描述初始化阶段。
