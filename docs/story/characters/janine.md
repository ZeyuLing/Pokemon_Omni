# 阿杏

<!-- GENERATED: edit content/story/characters.json and worldline.json -->

ID：`janine`；状态：候选。

阿桔之女的来源角色；道馆家系劲敌候选

出生工作年：。

## 人物形象



年龄阶段设计：

最终立绘／像素／3D 资产：已编入 GBA 的 80×80 人物立绘初版；见 assets/characters/manifest.json。行走、投球、背面及 3D 套件尚未完成；不能以此宣称最终美术完成。

## 原作身份参考（不计入本作生平）

金／银、心金／魂银、火红／叶绿、大师 EX：Janine／アンズ

- 游戏台词记载她是阿桔之女，并继承浅红道馆。
- 本轮亲属核对使用社区转录；大师 EX 官网另有她与叉字蝠的拍组记录，但那条公告不证明父女关系。

证据：游戏台词社区转录。原作资料不自动成为 Omni 已写生平；年龄、比赛履历与政治归属另行采用。

[来源 1](https://bulbapedia.bulbagarden.net/wiki/Janine/Quotes) · [来源 2](https://pokemonmasters-game.com/en-US/announcements/Update_4080_1W_7)

## 劲敌／关联人物提案

以道馆传承、毒属性状态与轮换消耗形成区别；若采用需另定阿桔当代职位，不能预先写成天王之女已获参赛权。

具体阵营、遇见节点、资格取得与结局未定。[选角研究](../rivals.md)。

## 已写生平

游戏立绘记录：[资产清单](../../../assets/characters/manifest.json)，角色键 `janine`。


## 完整生平的留白

| 阶段 | 正文 |
|---|---|
| 童年与成长 |  |
| 首次登场以前 |  |
| 已知事件之间的空白 |  |
| 后续人生 |  |
| 结局 |  |

## 人物关系


## 创作依据

- [35-kanto-rivals-and-succession.md](../../../docs/35-kanto-rivals-and-succession.md)
- [2026-09-26-kanto-rivals-evidence.md](../../../research/narrative/2026-09-26-kanto-rivals-evidence.md)
- [36-playable-opening-and-cast.md](../../../docs/36-playable-opening-and-cast.md)
