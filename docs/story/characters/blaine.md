# 夏伯

<!-- GENERATED: edit content/story/characters.json and worldline.json -->

ID：`blaine`；状态：已采用。

红莲火系训练与海岸事务负责人

出生工作年：。

## 人物形象

以动画未伪装时的秃顶、侧发、胡须为辨识基础；假发墨镜伪装另记，不将游戏立绘直接称作动画设计。1943 年年龄留空。

动画参考：[来源页面](https://www.serebii.net/anime/characters/blaine.shtml)；媒体 ID：`portrait-blaine`。本地原图经哈希锁定，不作为游戏成品立绘。

年龄阶段设计：

最终立绘／像素／3D 资产：

## 已写生平

- **1943-04 · 暗杀委托与双重清除**（正文已写）：会后收到包括两个人身份的行动命令，知情参与。 事件 ID：`assassination-orders`。
- **1943-05-15 · 海岸封锁与亚当死亡**（正文已写）：在海岸行动中知情参与，不能在后文抹去责任。 事件 ID：`cinnabar-betrayal`。

## 完整生平的留白

| 阶段 | 正文 |
|---|---|
| 童年与成长 |  |
| 首次登场以前 |  |
| 已知事件之间的空白 |  |
| 后续人生 |  |
| 结局 |  |

## 人物关系


## 创作依据

- [30-cinnabar-incident.md](../../../docs/30-cinnabar-incident.md)
