# 小驱

<!-- GENERATED: edit content/story/characters.json and worldline.json -->

ID：`chase`；状态：候选。

Let’s Go 男主角；关都选手候选

出生工作年：。

## 人物形象



年龄阶段设计：

最终立绘／像素／3D 资产：已编入 GBA 的 80×80 人物立绘初版；见 assets/characters/manifest.json。行走、投球、背面及 3D 套件尚未完成；不能以此宣称最终美术完成。

## 原作身份参考（不计入本作生平）

Let’s Go／大师 EX：Chase／カケル

- 大师 EX 官方介绍其与 Elaine 为朋友，来自关都。
- 小驱与步美分别作为候选管理，朋友关系不等于兄妹关系。

证据：官方介绍及社区名称索引。原作资料不自动成为 Omni 已写生平；年龄、比赛履历与政治归属另行采用。

[来源 1](https://www.pokemon.com/es/noticias-pokemon/ve-a-por-todas-con-chase-y-pikachu-en-pokemon-masters-ex) · [来源 2](https://wiki.pokemoncentral.it/Chase)

## 劲敌／关联人物提案

男主角改编为劲敌的备选；与步美分别建档，不认定兄妹，也不默认都完成原游戏旅程。

具体阵营、遇见节点、资格取得与结局未定。[选角研究](../rivals.md)。

## 已写生平

游戏立绘记录：[资产清单](../../../assets/characters/manifest.json)，角色键 `chase`。


## 完整生平的留白

| 阶段 | 正文 |
|---|---|
| 童年与成长 |  |
| 首次登场以前 |  |
| 已知事件之间的空白 |  |
| 后续人生 |  |
| 结局 |  |

## 人物关系


## 创作依据

- [35-kanto-rivals-and-succession.md](../../../docs/35-kanto-rivals-and-succession.md)
- [2026-09-26-kanto-rivals-evidence.md](../../../research/narrative/2026-09-26-kanto-rivals-evidence.md)
- [36-playable-opening-and-cast.md](../../../docs/36-playable-opening-and-cast.md)
