# 小明

<!-- GENERATED: edit content/story/characters.json and worldline.json -->

ID：`aj`；状态：候选。

非认证道馆训练家；民间劲敌候选

出生工作年：。

## 人物形象

参考初代动画第 8 集《The Path to the Pokémon League》的小明；本轮核对发行简介，具体外观和年龄适配待原片核验。

年龄阶段设计：

最终立绘／像素／3D 资产：已编入 GBA 的 80×80 人物立绘初版；见 assets/characters/manifest.json。行走、投球、背面及 3D 套件尚未完成；不能以此宣称最终美术完成。

## 原作身份参考（不计入本作生平）

初代电视动画第 8 集：A.J.／アキラ

- 经营未获联盟认可的道馆，目标百连胜后外出收集徽章。
- 民间连胜不是本作世界赛要求的天王／冠军级官方记录。

证据：授权发行平台简介。原作资料不自动成为 Omni 已写生平；年龄、比赛履历与政治归属另行采用。

[来源 1](https://tv.apple.com/us/episode/the-path-to-the-pokemon-league/umc.cmc.6hz6lk5ui76ufi4cdvd7a2kvx)

## 劲敌／关联人物提案

强调训练纪律和实战，适合早中期反复交手及民间挑战线；不凭未认证道馆推定其违法。

具体阵营、遇见节点、资格取得与结局未定。[选角研究](../rivals.md)。

## 已写生平

游戏立绘记录：[资产清单](../../../assets/characters/manifest.json)，角色键 `aj`。


## 完整生平的留白

| 阶段 | 正文 |
|---|---|
| 童年与成长 |  |
| 首次登场以前 |  |
| 已知事件之间的空白 |  |
| 后续人生 |  |
| 结局 |  |

## 人物关系


## 创作依据

- [35-kanto-rivals-and-succession.md](../../../docs/35-kanto-rivals-and-succession.md)
- [2026-09-26-kanto-rivals-evidence.md](../../../research/narrative/2026-09-26-kanto-rivals-evidence.md)
- [36-playable-opening-and-cast.md](../../../docs/36-playable-opening-and-cast.md)
