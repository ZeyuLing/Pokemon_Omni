...
> 本文为首轮实现记录。后续已补充 15 页游戏内图鉴、道具长文本与全部现有图像变体；最新容量、操作、验收和未完成项以 [游戏内图鉴状态](15-in-game-pokedex-status.md) 为准。

后续进展：图鉴现已嵌入可玩的真新镇开场，见 [真新镇游戏基础版](14-pallet-town-game.md)。本文描述的独立图鉴 ROM 保留为联调工具；游戏入口为 `/play`，独立图鉴仍在 `/gba`。
