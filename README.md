...
运行本地服务后打开 **`http://127.0.0.1:4173/play`**。调试版启动即打开游戏内图鉴，无需先领取。图鉴列表按 B 返回标题，再开始或继续冒险。新冒险为少年小智领取皮卡丘，已可探索真新镇、一号道路、常青市，捕捉伙伴、阻止中心的火箭队、购物并送回博士包裹。原生 ROM 为 `build/pallet/omni-pallet.gba`（32 MiB），九个场景；**完整关都一周目尚未完成**。具体操作、验收及战斗简化边界见 [关都一周目开发记录](docs/22-kanto-first-journey.md)。
...
战争序章《未竟的和平》已改为六场人物演绎（走位、对白、关屏、镜头），音乐换为归档火箭队 ROM 中经曲目核对的经典配乐；保留 22 人立绘画册：[观看序章](http://127.0.0.1:4173/play?opening)、[查看画册](http://127.0.0.1:4173/play?cast)。ROM 标题 L 回放、R 画册；探索 SELECT 切换 1／2／4 倍行走，BGM 由独立硬件计时驱动。任意外部模拟器快进不在原速音乐保证内。角色完整行走／战斗动作仍有未完成项，见 [实际实现与验收边界](docs/36-playable-opening-and-cast.md)。
