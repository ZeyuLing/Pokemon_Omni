...
运行本地服务后打开 **`http://127.0.0.1:4173/play`**。当前调试版启动即打开游戏内图鉴，无需先领取。图鉴列表按 B 返回标题，再开始或继续冒险；仍可探索真新镇、选择初始伙伴、进行练习战，并使用队伍、背包和保存。原生 ROM 为 `build/pallet/omni-pallet.gba`（20 MiB），已经通过实际 mGBA 按键流程；操作、构建、共享核心和边界见 [真新镇游戏基础版](docs/14-pallet-town-game.md)。
...
图鉴核心、中文内容编译和交互预览已实现。特殊形态独立展示与登记；预览调用与 GBA 同源的 C 核心。当前参考目录覆盖 2,384 个物种／形态／状态条目，含 1,025 个全国编号物种、97 条 Mega、34 条超极巨化、63 个霜奶仙组合及 23 条作者资料支持的羁绊参考。已与 1,579 条 PokeAPI 形态记录、1,302 条日本官方图鉴记录全部对账。23 条羁绊已核对用户与作者 ROM 的物种记录并提取正背面图，精确版本、招式及项目规则缺口仍保留；详见 [实现与验收](docs/12-pokedex-implementation.md)。
...
独立 GBA 图鉴已经链接为真实的 14 MiB ROM，包含中文界面、2,384 条目录、离线图片、招式与培养页、形态筛选和双槽 SRAM 保存，已在 mGBA 验证。生成后可从网站 `/gba` 直接运行或下载，完整冒险游戏和真机验证仍待完成。
