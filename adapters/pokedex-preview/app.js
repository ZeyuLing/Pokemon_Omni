...
function query(){
 ...
 total=core.dex_query_all(+$('category').value,+$('generation').value,+$('type').value,+$('progress').value,$('research').checked?1:0,numeric);
 const filtered=OmniFeatures.transform(Array.from(new Uint16Array(core.memory.buffer,core.dex_result_ptr(),total)));total=filtered.length;
 if(offset>=total&&offset){offset=0;return query();}
 const indices=filtered.slice(offset,offset+40),n=indices.length;
...
function select(index){
 ...
 <section class="section"><div class="move-toolbar"><h3>招式来源参考 <span id="move-count"></span></h3><label>筛选招式<input id="move-search" type="search" placeholder="招式中文或英文名" maxlength="80"></label></div><p class="facts">含前置进化与基础形态来源；来源并集不代表四个招式可以同时合法使用。数字为世代，L 为升级、M 为机器、T 为教学、E 为蛋招式、S 为活动。</p><div class="move-filters"><label>招式属性<select id="move-type"><option value="">全部</option>${data.types.map(t=>`<option value="${t.id}">${t.name}</option>`).join('')}</select></label><label>分类<select id="move-kind"><option value="">全部</option><option value="Physical">物理</option><option value="Special">特殊</option><option value="Status">变化</option></select></label><label>来源世代<select id="move-gen"><option value="">全部</option>${Array.from({length:9},(_,i)=>`<option>${i+1}</option>`).join('')}</select></label></div><div id="moves"></div></section>`;
 renderArt(e);
 for(const id of ['art-shiny','art-female','art-back'])if($(id))$(id).onchange=()=>renderArt(e);
 for(const b of $('detail').querySelectorAll('[data-related]'))b.onclick=()=>select(+b.dataset.related);
 OmniFeatures.mount(e);progressButtons();moves();$('move-search').oninput=()=>{moveLimit=15;moves();};for(const id of ['move-type','move-kind','move-gen'])$(id).onchange=()=>{moveLimit=15;moves();};
 if($('hp-comparison')){for(const id of ['hp-level','hp-iv','hp-ev','hp-dynamax'])$(id).oninput=updateHp;updateHp();}
}
function romNotes(e){
 const differences=e.rom_evidence?.document_differences;if(!differences)return '';
 const notes=[];
 if(differences.types)notes.push('属性：作者文档为 '+differences.types.document.map(typeName).join('／')+'；两份 ROM 均为 '+differences.types.rom.map(typeName).join('／')+'。');
 if(differences.stats){const names={hp:'HP',atk:'攻击',def:'防御',spa:'特攻',spd:'特防',spe:'速度'};for(const k of Object.keys(names))if(differences.stats.document[k]!==differences.stats.rom[k])notes.push(names[k]+'：作者文档 '+differences.stats.document[k]+'，两份 ROM 均为 '+differences.stats.rom[k]+'。');}
 return notes.length?'<p class="note rom-discrepancy">文档与游戏数据差异：'+escape(notes.join(' '))+' 当前按两份 ROM 一致的数据展示。</p>':'';
}
function artProvenance(e){
 const art=e.art_reference;
 if(art?.status==='rom_extracted')return '<span>游戏内原始像素图 · 用户 ROM 与作者发布包一致</span>'+(art.normal_shiny_identical?'<span>此版本普通与异色调色板相同</span>':'');
 if(art?.status==='source_rom_extracted')return '<span>游戏内原始像素图 · 究极绿宝石用户版本</span>'+(art.normal_shiny_identical?'<span>此版本普通与异色调色板相同</span>':'');
...
function artControls(e){
 ...
 return `<div class="facts art-controls">${v.back_default?'<label><input id="art-back" type="checkbox"> 背面</label>':''}${v.front_shiny?'<label><input id="art-shiny" type="checkbox"> 异色参考图</label>':''}${v.front_female?'<label><input id="art-female" type="checkbox"> 雌性参考图</label>':''}${e.category==='dynamax'?'<span>图片为极巨化前的对应形态</span>':''}${artProvenance(e)}</div>`;
}
function renderArt(e){
 ...
 const variant=$('art-back')?.checked?(shiny?'back_shiny':'back_default'):shiny?(female?'front_shiny_female':'front_shiny'):(female?'front_female':'front_default');
 ...
 if(!url)return;
 const img=new Image();img.alt=e.name_zh_hans+(shiny?'异色':'')+(female?'雌性':'')+($('art-back')?.checked?'背面':'正面')+'参考图';img.width=135;img.height=135;img.referrerPolicy='no-referrer';
 const fallback=(slow=false)=>{if(portrait.artToken!==token)return;const note=portrait.querySelector('.image-note');if(note){note.textContent=slow?'参考图加载较慢，仍在尝试…':e.art_reference?.local_only?'本地 ROM 素材缺失或读取失败，请先运行素材提取脚本。':'参考图暂不可用：远程图片加载失败';if(!slow){const retry=document.createElement('button');retry.textContent='重试图片';retry.onclick=()=>renderArt(e);note.append(document.createElement('br'),retry);}}};
