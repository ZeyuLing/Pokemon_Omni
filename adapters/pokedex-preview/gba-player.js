(async()=>{
 ...
 try{
  ...
  const audioPtr=m._malloc(8192);let audioContext=null,audioOn=false,audioAt=0;
  function drainAudio(){let count;while((count=m._mgbawasm_read_audio(audioPtr,2048))>0){if(!audioOn||!audioContext||audioContext.state!=='running')continue;const buffer=audioContext.createBuffer(2,count,m._mgbawasm_sample_rate()),start=audioPtr>>1;for(let ch=0;ch<2;ch++){const out=buffer.getChannelData(ch);for(let i=0;i<count;i++)out[i]=m.HEAP16[start+i*2+ch]/32768;}const src=audioContext.createBufferSource();src.buffer=buffer;src.connect(audioContext.destination);audioAt=Math.max(audioAt,audioContext.currentTime);if(audioAt>audioContext.currentTime+0.25)audioAt=audioContext.currentTime;src.start(audioAt);audioAt+=count/buffer.sampleRate;}}
  const audioButton=document.getElementById('gba-audio');if(audioButton)audioButton.onclick=async()=>{try{if(!audioContext)audioContext=new AudioContext();await audioContext.resume();audioOn=!audioOn;audioButton.textContent=audioOn?'关闭音效':'开启音效';}catch{status.textContent='此浏览器无法播放音效，游戏可继续运行。';}};
  function loop(time){if(!last)last=time;acc+=Math.min(time-last,100);last=time;if(!paused){while(acc>=1000/60){m._mgbawasm_run_frame();drainAudio();acc-=1000/60;}frame.data.set(m.HEAPU8.subarray(m._mgbawasm_video_ptr(),m._mgbawasm_video_ptr()+240*160*4));ctx.putImageData(frame,0,0);}else acc=0;if(time-saveAt>5000){save();saveAt=time;}requestAnimationFrame(loop);}
