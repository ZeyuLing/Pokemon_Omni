...
static void paragraph(const char *s){unsigned line=0,x=4;while(*s){uint32_t c=utf8(&s);unsigned w=c<128?8:16;if(c=='\n'){++line;x=4;continue;}if(x+w>236){++line;x=4;}if(line>=text_scroll&&line<text_scroll+6)glyph((int)x,24+(int)(line-text_scroll)*18,c,INK);x+=w;}text_lines=(uint16_t)(line+1);if(text_scroll>=text_lines)text_scroll=text_lines-1;}
...
static void draw_images(void){static const char *names[]={"正面","背面","异色正面","异色背面","雌性正面","雌性背面","异色雌性正面","异色雌性背面"};title("图像变体");text(4,23,names[image_variant],INK,237);picture_offset(gba_variants[entry][image_variant],88,44);text(4,110,"不存在的变体不会补画替代",MUTED,237);text(4,126,"原图相同会显示相同图像",MUTED,237);footer("上下切换 SELECT目录 B返回");}
...
static void draw_profile(void){const OmniDexEntry *e=&omni_pokedex_catalog.entries[entry];const OmniDexProfile *p=&omni_pokedex_profiles[entry];static const char *labels[]={"HP","攻击","防御","特攻","特防","速度"};unsigned i;title(e->name_zh);text(4,24,gba_info[entry].types,TEAL,160);text(4,43,gba_info[entry].abilities,INK,160);text(4,62,gba_info[entry].category,MUTED,160);picture(entry,171,26);for(i=0;i<6;++i){int x=(i%2)*120,y=91+(int)(i/2)*16;text(x+5,y,labels[i],MUTED,x+47);num(x+52,y,p->stats[i],INK);}footer("左右翻页 SELECT目录 B返回");}
...
void omni_game_dex_open_entry(uint32_t id){int32_t found=omni_dex_find(&omni_pokedex_catalog,id);omni_game_dex_open();if(found>=0){entry=(uint16_t)found;screen=DETAIL;page=scroll=plan_cursor=text_scroll=0;move_detail=image_variant=0;}}
