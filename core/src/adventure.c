...
static int valid(const OmniAdventure *s){
 unsigned i;if(!s||s->location<1||s->location>9||s->chapter>3||s->starter>4||s->party_count>6||s->pc_claimed>1||s->events>15||s->potions>999||s->balls>999||s->money>999999||s->battles_won>s->battles_played||s->badges>8||s->play_seconds>3599999)return 0;
