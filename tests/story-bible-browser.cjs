...
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 assert(timelineText.includes('1974 年 4 月 1 日')&&timelineText.includes('仅大木博士知晓'));
 assert(!timelineText.includes('T=0 对应本作 1967 年'));
 assert(timelineText.includes('1975-09-01')&&timelineText.includes('1975-10-01'));
 assert((await page.locator('#viridian-center time').innerText()).includes('1974-04-02 至 1974-04-05'));
 assert(await page.locator('#first-world-tournament').innerText().then(s=>s.includes('已定日程／情节未写')));
 await page.setViewportSize({width:1400,height:1000});await page.screenshot({path:path.join(root,'worldline-opening.png')});
 assert.equal(await page.locator('.event').count(),32);
