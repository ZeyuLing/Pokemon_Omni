...
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await page.screenshot({path:'build/pallet/browser-cover.png',fullPage:true});await key('KeyX');await page.screenshot({path:'build/pallet/browser-save-menu.png',fullPage:true});await key('KeyZ');await key('Enter');await key('KeyZ');await key('KeyZ');await key('Enter');await page.screenshot({path:'build/pallet/browser-menu.png',fullPage:true});
 ...
 await page.locator('#gba-import').setInputFiles('build/pallet/test-game.sav');await page.getByText('已载入 SRAM；游戏将验证存档槽。',{exact:true}).waitFor();await page.waitForTimeout(400);await key('KeyX');await key('KeyZ');await page.screenshot({path:'build/pallet/browser-lab.png',fullPage:true});
 const prior=await page.evaluate(()=>localStorage.getItem('omni-pallet-sram-v1'));
 await page.locator('#gba-import').setInputFiles({name:'wrong.sav',mimeType:'application/octet-stream',buffer:Buffer.alloc(32768)});await page.getByText('SRAM 缺少匹配的有效存档槽，当前进度保持不变',{exact:true}).waitFor();assert.equal(await page.evaluate(()=>localStorage.getItem('omni-pallet-sram-v1')),prior);
 await page.locator('#gba-audio').click();await page.getByRole('button',{name:'关闭音乐与音效',exact:true}).waitFor();await page.locator('#gba-audio').click();
