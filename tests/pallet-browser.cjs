...
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await page.screenshot({path:'build/pallet/browser-debug-dex.png',fullPage:true});await key('KeyX');await key('KeyZ');await key('KeyZ');await key('Enter');await page.screenshot({path:'build/pallet/browser-menu.png',fullPage:true});
