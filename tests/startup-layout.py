...
manifest = json.loads((ROOT/'assets/source/title-screen.json').read_text('utf8'))
tx,ty,tw,th = manifest['prompt_layout']
pixels = [tuple((v<<3)|(v>>2) for v in (c&31,(c>>5)&31,(c>>10)&31))+(255,)
          for (c,) in struct.iter_unpack('<H', (OUT/'title.bin').read_bytes()[:240*160*2])]
compared = 0
for y in range(160):
    for x in range(240):
        if tx <= x < tx+tw and ty <= y < ty+th: continue
        assert cover.getpixel((x,y)) == pixels[y*240+x], 'ROM cover differs from compiled art'
        compared += 1
assert any(cover.getpixel((x,y)) != pixels[y*240+x] for y in range(ty,ty+th) for x in range(tx,tx+tw)), 'Source PRESS START not visible'
...
report = {'selected_new_game_panel': {'pixels_compared': 7680, 'differences': 0,
          ...
          'cover': {'source': manifest['art']['path'], 'pixels_compared_outside_prompt': compared,
                    'differences': 0, 'prompt_layout': manifest['prompt_layout'],
                    'scope': 'Original generated cover in real ROM; source-derived PRESS START blink checked separately.'},
