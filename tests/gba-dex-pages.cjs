...
module.exports=function checkDex(m,press,shot){
 ...
 function pixels(variant){const s=state(),url=catalog.entries[s.entry].art_reference.variants[variant];assert(url);const row=report.images.find(r=>r.url===url);let at=row.offset,mode=atlas.readUInt16LE(at);at+=2;const values=[];while(values.length<4096){let n=1;if(mode){n=atlas.readUInt16LE(at);at+=2;}const c=atlas.readUInt16LE(at);at+=2;for(let i=0;i<n;i++)values.push(c);}const v=m._mgbawasm_video_ptr();for(let i=0;i<4096;i++){const c=values[i]&0x8000?30|(30<<5)|(27<<10):values[i],p=v+((44+(i>>6))*240+88+i%64)*4;assert.equal(m.HEAPU8[p]>>3,c&31);assert.equal(m.HEAPU8[p+1]>>3,(c>>5)&31);assert.equal(m.HEAPU8[p+2]>>3,(c>>10)&31);}}
