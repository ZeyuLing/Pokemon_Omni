...
try {
 ...
 & $Zig cc @debugFlags '-DOMNI_AUDIO_FAST=__attribute__((section(".iwram"),target("arm")))' -target arm-freestanding-eabi -mcpu=arm7tdmi -mthumb -std=c99 -O2 -ffreestanding -fno-builtin -fno-unwind-tables -fno-asynchronous-unwind-tables -nostdlib -Icore/include -Icontent/pokedex/generated -Icontent/training/generated -Ibuild/gba -Ibuild/pallet -Iadapters/gba adapters/gba/start.s adapters/gba/pallet_game.c adapters/gba/music.c adapters/gba/pokedex_game.c core/src/presentation.c core/src/stage.c core/src/audio.c build/pallet/rocket_text.c build/pallet/rocket_text.s build/pallet/title.s build/pallet/classic_audio.c build/pallet/classic_audio.s build/pallet/opening_stage.c build/pallet/opening_stage.s build/pallet/presentation_data.c build/pallet/cast.s core/src/adventure.c core/src/pokedex.c core/src/training.c core/src/battle_stats.c content/pokedex/generated/catalog.c content/training/generated/plans.c build/gba/gba_data.c build/gba/blobs.s build/pallet/world_data.c build/pallet/world_blobs.s build/pallet/game_ui.s '-Wl,-T,adapters/gba/rom.ld' -o "$output/omni-pallet.elf"
