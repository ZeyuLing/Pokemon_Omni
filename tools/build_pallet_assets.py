...
def main():
    ...
    names={'gTileset_General':'primary/general','gTileset_Building':'primary/building','gTileset_PalletTown':'secondary/pallet_town','gTileset_GenericBuilding1':'secondary/generic_building_1','gTileset_GenericBuilding2':'secondary/generic_building_2','gTileset_Lab':'secondary/lab'}
    blob=bytearray();map_rows=[];actor_rows=[];sign_rows=[];warp_rows=[];audit=[]
    sprites=['red_normal','mom','prof_oak','blue','woman_1','fat_man','scientist','daisy','item_ball']
    ...
    for source,m in maps.items():
        ...
        bg=Image.new('RGB',(w*16,h*16),palettes[0][0]);mask=bytearray(w*h*256);collision=[]
        ...
        map_rows.append('{'+','.join([str(m['id']),str(w),str(h),cs(m['name']),str(art_offset),str(mask_offset),str(collision_offset),str(actor_start),str(len(m['actors'])),str(sign_start),str(len(m['signs'])),str(warp_start),str(len(meta['warp_events']))])+'}')
        bg.save(OUT/(source+'.png'))
        audit.append({'id':m['id'],'key':m['key'],'source':source,'width':w,'height':h,'collision':collision,'warps':meta['warp_events'],'actors':m['actors'],'signs':m['signs']})
    sprite_rows=[]
    for sprite in sprites:
        path='graphics/object_events/pics/'+('misc/item_ball.png' if sprite=='item_ball' else f'people/{sprite}.png')
...
#include <stdint.h>
typedef struct {uint8_t id,w,h;const char *name;uint32_t art,mask,collision;uint16_t actors,actor_count,signs,sign_count,warps,warp_count;} PalletMap;
...
extern const PalletMap pallet_maps[5];
extern const PalletActor pallet_actors[];
extern const PalletSign pallet_signs[];
extern const PalletWarp pallet_warps[];
extern const PalletSprite pallet_sprites[9];
extern const uint32_t pallet_battle_sprites[6];
...
''')
    ...
    code+='const uint32_t pallet_battle_sprites[6]={'+','.join(map(str,battle_offsets))+'};\n'
