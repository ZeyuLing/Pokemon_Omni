...
foreach ($file in $jsonFiles) { $null = Get-Content -Raw -LiteralPath $file.FullName | ConvertFrom-Json }
$project = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'config/project.json') | ConvertFrom-Json
...
$rules = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'config/rulesets.json') | ConvertFrom-Json
...
$difficulty = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'config/difficulty.json') | ConvertFrom-Json
...
$roster = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'content/bond/rocket-2.1-roster.json') | ConvertFrom-Json
if ($roster.forms.Count -ne 23 -or $roster.complete_version_verified) { throw 'Bond research provenance changed; review required.' }
$dexConfig = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'config/pokedex.json') | ConvertFrom-Json
if ($dexConfig.special_forms_are_standalone_entries -ne $true) { throw 'Special forms must be full Pokedex entries.' }
$dex = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'content/pokedex/entries.json') | ConvertFrom-Json
...
$regions = (Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'content/regions/index.json') | ConvertFrom-Json).regions
if (@($regions.id | Select-Object -Unique).Count -ne $regions.Count) { throw 'Duplicate region IDs.' }
$quest = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'content/examples/partner-introduction.json') | ConvertFrom-Json
$bindings = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'adapters/examples/partner-introduction.bindings.json') | ConvertFrom-Json
