"""Finalize the standard GBA cartridge header and record the actual build size."""
from pathlib import Path
import argparse,hashlib,json
parser=argparse.ArgumentParser()
parser.add_argument('--rom',default='build/gba/omni-dex.gba')
parser.add_argument('--title',default='OMNI DEX')
parser.add_argument('--code',default='OMDE')
parser.add_argument('--scope',default='Standalone GBA Pokedex integration cartridge, not the full world game.')
parser.add_argument('--asset',action='append',default=[])
args=parser.parse_args()
p=Path(args.rom)
b=bytearray(p.read_bytes())
for asset in ('art.bin','font.bin'):
    assert Path('build/gba',asset).read_bytes() in b, 'Linked ROM contains stale embedded asset: '+asset
for asset in args.asset:
    assert Path(asset).read_bytes() in b, 'Linked ROM contains stale embedded asset: '+asset
b[4:0xa0]=bytes.fromhex('')
assert len(args.title.encode('ascii'))<=12 and len(args.code)==4
b[0xa0:0xac]=args.title.encode('ascii').ljust(12,b' ')
b[0xac:0xb0]=args.code.encode('ascii')
b[0xb0:0xb2]=b'00'
b[0xb2]=0x96
b[0xb3:0xbd]=bytes(10)
b[0xbd]=(-sum(b[0xa0:0xbd])-0x19)&255
assert len(b)<=32*1024*1024
# Pad to the next MiB, with erased-ROM bytes, rather than pretending to fill 32 MiB.
b+=bytes([255])*((-len(b))%(1024*1024))
p.write_bytes(b)
report={'rom':p.as_posix(),'bytes':len(b),'sha256':hashlib.sha256(b).hexdigest(),'header_checksum_valid':((-sum(b[0xa0:0xbd])-0x19)&255)==b[0xbd],'scope':args.scope}
(p.parent/'build-report.json').write_text(json.dumps(report,indent=2)+'\n')
print(json.dumps(report))
